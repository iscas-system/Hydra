...
type MinCostAlgo interface {
	MinCost(costSolver CostSolver, gpu types.GPU, jobs []types.Job) (float64, []types.Job)
}
...
type MinCostByBranchAndBound struct {
	LCStandard MinCostBranchAndBoundLCStandard
}

func NewMinCostByBranchAndBoundAlgo(LCStandard MinCostBranchAndBoundLCStandard) MinCostAlgo {
	return &MinCostByBranchAndBound{LCStandard: LCStandard}
}

func (m *MinCostByBranchAndBound) MinCost(costSolver CostSolver, gpu types.GPU, jobs []types.Job) (float64, []types.Job) {
...
// MinCost 一个分支限界搜索方法，但是将所有不具有ddl的任务直接按照SRJF的顺序固定住，
